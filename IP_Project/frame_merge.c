...
void frame_merge_sw(u32* ddr_Addr, u32 Draw_Addr, u32 Video_Addr, u32 Comp_Addr, u32 hres, u32 vres)
{
	...
	int i, j;
	int draw_frame_pixel, video_frame_pixel, comp_frame_pixel;
	for (j = 0; j < vres; j++)
	{
		for(i = 0; i < hres; i++)
		{
			// 1) Read DrawFrame and VideoFrame from DDR memory
			draw_frame_pixel = ddr_Addr[Draw_Addr + j*hres + i];
			video_frame_pixel = ddr_Addr[Video_Addr + j*hres + i];

			// 2) Evaluate new pixel
			if (FM_WHITE != draw_frame_pixel)
			{
			...
			ddr_Addr[Comp_Addr + j*hres + i] = comp_frame_pixel;
		}
	}
