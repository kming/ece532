...
// Date:              Thu Jan 30 03:14:01 2014 (by Create and Import Peripheral Wizard)
...
//----------------------------------------
  ...
  assign mst_cntl_rd_req   = 1'b0; // mst_reg[0][0];    --wrahman
  assign mst_cntl_wr_req   = 1'b1; // mst_reg[0][1];    --wrahman
  assign mst_cntl_bus_lock = 1'b0; //mst_reg[0][2];     --wrahman
  assign mst_cntl_burst    = 1'b1; //mst_reg[0][3];    --wrahman
  assign mst_ip2bus_addr   = slv_reg0[C_MST_AWIDTH-1:0]; //{mst_reg[7], mst_reg[6], mst_reg[5], mst_reg[4]};    --wrahman    // use address width
  assign mst_ip2bus_be     = 16'hFFFF; //{mst_reg[9], mst_reg[8]};    --wrahman, byte enable everything
  assign mst_xfer_reg_len  = 4*1280; //{mst_reg[14][3 : 0], mst_reg[13], mst_reg[12]};// changed to 20 bits     --wrahman BYTES_PER_PIXEL * PIXELS_PER_LINE
  assign mst_xfer_length   = mst_xfer_reg_len[C_LENGTH_WIDTH-1 : 0];

  wire overlay_go;   // --wrahman
  assign overlay_go = slv_reg1[0];	// slave reg 1 is go register for software to start this IP
  ...
  always @ ( posedge Bus2IP_Clk)
    begin
      if (Bus2IP_Resetn == 1'b0 || mst_cmd_sm_clr_go == 1'b1 )
        ...
        //if ( mst_cmd_sm_busy == 1'b0 && mst_byte_we[GO_BYTE_LANE] == 1'b1 && Bus2IP_Data[(GO_BYTE_LANE-(GO_BYTE_LANE/BE_WIDTH)*BE_WIDTH)*8 +: 8] == GO_DATA_KEY)  -- wrahman
        if (mst_cmd_sm_busy == 1'b0 && overlay_go)
          begin
            mst_go   <= 1'b1;
          end
    end
 ...
 assign ip2bus_mstwr_eof_n     = !mst_llwr_sm_eof;
  ...
  // FIFO depth is 128 words. User can modify the depth based on their requirement.
   ...
   DATA_CAPTURE_FIFO_I (
     ...
     //.Data_Out(ip2bus_mstwr_d), -- wrahman, the example loops back readdata to writedata, we don't want that
     .Data_out(),
     .FIFO_Full(),
     .FIFO_Empty(),
     .Addr()); // DATA_CAPTURE_FIFO_I
 
    assign ip2bus_mstwr_d = 32'hFF00FF00;	// --wrahman, assign a constant pixel colour value for now...
  // ------------------------------------------------------------
