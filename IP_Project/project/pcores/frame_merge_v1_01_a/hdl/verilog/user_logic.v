...
// Date:              Fri Jan 31 01:46:01 2014 (by Create and Import Peripheral Wizard)
...
parameter HRES                          = 20'd1280;      // length of mst_xfer_reg_len = 20b
parameter VRES                          = 20'd720;
parameter BYTES_PER_PIXEL               = 20'd4;
...
//----------------------------------------------------------------------------
  ...
  wire                                      draw_fifo_sel;
  wire                                      draw_fifo_read;
  wire      [C_MST_NATIVE_DATA_WIDTH-1 : 0] mem2ip_draw_fifo_data;
  
  wire                                      video_fifo_sel;
  wire                                      video_fifo_read;
  wire      [C_MST_NATIVE_DATA_WIDTH-1 : 0] mem2ip_video_fifo_data;

  // Core controller to master signals
  wire                                      frame_merge_rd_req;
  wire                                      frame_merge_wr_req;
  wire      [C_MST_AWIDTH-1 : 0]            frame_merge_addr;
  wire                                      frame_merge_go;

  wire                                      comp_fifo_write;
  wire													comp_fifo_sel;
  wire      [C_MST_NATIVE_DATA_WIDTH-1 : 0] ip2mem_comp_fifo_data;
  wire                                      comp_fifo_full;
  wire                                      comp_fifo_empty;
  wire                                      draw_fifo_full;
  wire                                      draw_fifo_empty;
...
// signals for master model write locallink interface state machine
 ...
 parameter                                  GO_BYTE_LANE = 15;
  ...
  localparam    PIXEL_LINE_FIFO_SIZE = 'd1400;    // pad fifos since the empty and full flags are unreliable, max is 1280

    frame_merge_core #(
        .HRES(HRES),
        .VRES(VRES),
        .DWIDTH(C_MST_NATIVE_DATA_WIDTH),
        .AWIDTH(C_MST_AWIDTH),
        .SLV_REG_DWIDTH(C_SLV_DWIDTH),
        .BYTES_PER_PIXEL(BYTES_PER_PIXEL)
    ) frame_merge_core_inst0
    (
        // Inputs
        .clk(Bus2IP_Clk),
        .reset(bus2ip_Reset),
        .draw_fifo_data_in(mem2ip_draw_fifo_data),
        .video_fifo_data_in(mem2ip_video_fifo_data),
        .comp_fifo_full(comp_fifo_full),
        .comp_fifo_empty(comp_fifo_empty),
        .draw_fifo_full(draw_fifo_full),
        .draw_fifo_empty(draw_fifo_empty),
        .video_fifo_full(video_fifo_full),
        .video_fifo_empty(video_fifo_empty),

        // Outputs
        .draw_fifo_sel(draw_fifo_sel),
        .draw_fifo_read(draw_fifo_read),
        .video_fifo_sel(video_fifo_sel),
        .video_fifo_read(video_fifo_read),
        .comp_fifo_write(comp_fifo_write),
        .comp_fifo_data_out(ip2mem_comp_fifo_data),
		  .comp_fifo_sel(comp_fifo_sel),

        // Master Controller signals
        .master_busy(mst_cmd_sm_busy),
		  .bus_ack(bus2ip_mst_cmdack),
        .rd_req(frame_merge_rd_req),
        .wr_req(frame_merge_wr_req),
        .addr(frame_merge_addr),
        .go(frame_merge_go),

        // Slave regs
        .slave_go(slv_reg0),
        .slave_draw_addr(slv_reg1),
        .slave_video_addr(slv_reg2),
        .slave_comp_addr(slv_reg3)
    );
...
//----------------------------------------
  ...
  assign mst_cntl_rd_req   = frame_merge_rd_req;    // --wr, change
  assign mst_cntl_wr_req   = frame_merge_wr_req;
  //assign mst_cntl_wr_req   = 1'b0;
  assign mst_cntl_bus_lock = 1'b0;                  // --wr, burst lock? ignore...
  assign mst_cntl_burst    = 1'b1;
  assign mst_ip2bus_addr   = frame_merge_addr;      // --wr, 4x1byte regs = 32 bit address = C_MST_AWIDTH
  assign mst_ip2bus_be     = 16'hFFFF;              // --wr, 2x1byte = 16 bit field
  //assign mst_xfer_reg_len  = {mst_reg[14][3 : 0], mst_reg[13], mst_reg[12]};// changed to 20 bits 
  assign mst_xfer_reg_len  = slv_reg4[C_LENGTH_WIDTH-1 : 0];//4*1280;  // --wr, get entire line
  assign mst_xfer_length   = mst_xfer_reg_len[C_LENGTH_WIDTH-1 : 0];    // --wr, should this be of length 20, like mst_xfer_reg_len?
  ...
  always @ ( posedge Bus2IP_Clk)
    begin
      if (Bus2IP_Resetn == 1'b0 || mst_cmd_sm_clr_go == 1'b1 )
        ...
        //if ( mst_cmd_sm_busy == 1'b0 && slv_reg0[0])    // --wr, change
        if ( mst_cmd_sm_busy == 1'b0 && frame_merge_go)    // --wr, change
          begin
            mst_go   <= 1'b1;
          end
    end
     ...
     if (Bus2IP_Resetn == 1'b0 )
       ...
       begin
         ...
         case (mst_llrd_sm_state)
             ...
             CMD_IDLE:      // --wr, looks like bug, should be LLRD_IDLE?
 ...
 assign ip2bus_mstwr_eof_n     = !mst_llwr_sm_eof;
  ...
  assign bus2ip_Reset   = !Bus2IP_Resetn;
    ...
    srl_fifo_f #(
        ...
        .C_DEPTH(PIXEL_LINE_FIFO_SIZE)      // Fifo not that reliable on overflow, so pad it
    ) DRAWFRAME_CAPTURE_FIFO_I 
    (
        .Clk(Bus2IP_Clk),
        .Reset(bus2ip_Reset),
        .FIFO_Write(mst_fifo_valid_write_xfer & draw_fifo_sel),    // Merger FSM decides if reading draw frame or video-in frame
        .Data_In(bus2ip_mstrd_d),
        .FIFO_Read(draw_fifo_read),
        .Data_Out(mem2ip_draw_fifo_data),
        .FIFO_Full(draw_fifo_full),
        .FIFO_Empty(draw_fifo_empty),
        .Addr()
    );
    ...
    srl_fifo_f #(
        .C_DWIDTH(C_MST_NATIVE_DATA_WIDTH),
        .C_DEPTH(PIXEL_LINE_FIFO_SIZE)      // Fifo not that reliable on overflow, so pad it
    ) VIDEOFRAME_CAPTURE_FIFO_I 
    (
        .Clk(Bus2IP_Clk),
        .Reset(bus2ip_Reset),
        .FIFO_Write(mst_fifo_valid_write_xfer & video_fifo_sel),    // Merger FSM decides if reading draw frame or video-in frame
        .Data_In(bus2ip_mstrd_d),
        .FIFO_Read(video_fifo_read),
        .Data_Out(mem2ip_video_fifo_data),
    ...
    // CompositeFrame FIFO
    // this should be moved to frame_merge_core
    srl_fifo_f #(
        .C_DWIDTH(C_MST_NATIVE_DATA_WIDTH),
        .C_DEPTH(PIXEL_LINE_FIFO_SIZE)      // Fifo not that reliable on overflow, so pad it
    ) COMPFRAME_OUTPUT_FIFO_O
    (
        .Clk(Bus2IP_Clk),
        .Reset(bus2ip_Reset),
        .FIFO_Write(comp_fifo_write),
        .Data_In(ip2mem_comp_fifo_data),
        .FIFO_Read(mst_fifo_valid_read_xfer & comp_fifo_sel),
...
//assign ip2bus_mstwr_d = 32'hFF00FF00;
  // ------------------------------------------------------------
